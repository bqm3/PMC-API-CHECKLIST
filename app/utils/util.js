...
function checkDataExcel(data, index) {
  try {
    ...
    if (tenKhuVuc.includes("tầng")) {
      ...
      console.log("matchTang", matchTang);
      if (matchTang) {
        const expectedTang = matchTang[0]; // Tầng mong đợi từ tên tầng

        // Kiểm tra xem tên hạng mục có chứa tầng mong đợi không
        if (
          !tenHangMuc.includes(expectedTang) ||
          !tenTang.includes(expectedTang)
        ) {
        //   throw new Error(`Lỗi dòng ${index}, dữ liệu tầng của khu vực và hạng mục không khớp với tên tầng`);
        }
      } else {
        console.log(
          "Lỗi: Không tìm thấy thông tin tầng trong tên khu vực",
          index
        );
        //   throw new Error('Lỗi: Không tìm thấy thông tin tầng trong tên khu vực.', index);
      }
    }
  } catch (err) {
    throw err;
